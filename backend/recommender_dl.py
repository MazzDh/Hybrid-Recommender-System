...
class DLRecommender:
    ...
    def get_user_embedding(self, user_id: int):
        ...
        # model.u_emb is the embedding layer
        with torch.no_grad():
            emb = self.model.u_emb(torch.tensor(idx))
        return emb.numpy()

    def get_item_embedding(self, item_name: str):
        if item_name not in self.item2idx:
            return None
        idx = self.item2idx[item_name]
        with torch.no_grad():
            emb = self.model.i_emb(torch.tensor(idx))
        return emb.numpy()
